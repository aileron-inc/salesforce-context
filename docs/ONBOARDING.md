...
- プレフィックス（`Account.`、`Object__c.` 等）は主オブジェクトに対しては剥がす
...
- 別オブジェクトの項目は主オブジェクトから直接取れないことがある。その場合は主従を逆にして親参照で取る（例: `FROM Contact ... Account.Field__c`）
- 同一オブジェクトをレコード種別で切り分ける場合は `RecordType.DeveloperName` で WHERE を書く。レコードタイプは `SELECT Id, Name, DeveloperName FROM RecordType WHERE SobjectType = '...'` で確認
